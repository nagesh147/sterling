...
def test_account_daily_pnl_is_authoritative_for_orb_safety(monkeypatch):
    monkeypatch.setattr(
        "app.services.kite_engine.state.daily_realized_pnl_strict",
        lambda uid: -1600.0,
    )
...
def test_daily_pnl_read_failure_blocks_closed(monkeypatch):
    def fail(_uid):
        raise RuntimeError("db unavailable")

    monkeypatch.setattr("app.services.kite_engine.state.daily_realized_pnl_strict", fail)
    live_safety.configure_daily_loss(
        live_safety.DailyLossConfig(soft_warn_inr=-1000.0, hard_halt_inr=-1500.0)
    )
    decision = live_safety.assert_safe_to_trade(
        positions=[], idempotency_key="orb-fail-closed", uid="user-3", check_daily_loss=True
    )
    assert decision.allowed is False
