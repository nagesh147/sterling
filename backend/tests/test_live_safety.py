...
class TestSafetyEndpoints:
    ...
    def test_get_kill_switch_default(self, client) -> None:
        r = client.get("/api/v1/trading/kill-switch")
        assert r.status_code == 200
        assert r.json()["enabled"] is False

    def test_set_kill_switch_then_get(self, client) -> None:
        r = client.post("/api/v1/trading/kill-switch",
                        json={"enabled": True, "reason": "via test"})
        assert r.status_code == 200
        body = r.json()
        assert body["enabled"] is True
        assert body["reason"] == "via test"

        r2 = client.get("/api/v1/trading/kill-switch")
        assert r2.json()["enabled"] is True

    def test_daily_loss_endpoint(self, client) -> None:
        r = client.get("/api/v1/trading/daily-loss")
        assert r.status_code == 200
        body = r.json()
        for key in ("pnl_usd", "level", "soft_warn_usd", "hard_halt_usd"):
            assert key in body

    def test_retry_queue_empty_initially(self, client) -> None:
        r = client.get("/api/v1/trading/retry-queue")
        assert r.status_code == 200
        assert r.json()["count"] == 0

    def test_retry_queue_lists_after_enqueue(self, client) -> None:
        item = enqueue_retry({"u": "BTC"}, error="x")
        r = client.get("/api/v1/trading/retry-queue")
        body = r.json()
        assert body["count"] == 1
        assert body["items"][0]["id"] == item.id

    def test_retry_queue_delete(self, client) -> None:
        item = enqueue_retry({"u": "BTC"}, error="x")
        r = client.delete(f"/api/v1/trading/retry-queue/{item.id}")
        assert r.status_code == 200
        r2 = client.delete(f"/api/v1/trading/retry-queue/{item.id}")
