...
@pytest.mark.asyncio
async def test_adapter_funding_rate_caches():
    from app.services.exchanges.adapters.delta_india import DeltaIndiaAdapter
    a = DeltaIndiaAdapter(api_key="x", api_secret="y", is_paper=True)
    a._public_get = AsyncMock(return_value={"result": {"rate": 0.00012}})

    r1 = await a.get_funding_rate(27)
    r2 = await a.get_funding_rate(27)
    assert r1["funding_rate_8h_pct"] == 0.00012
    assert r2["source"] == "cache"
    # _public_get only called once
    assert a._public_get.call_count == 1


@pytest.mark.asyncio
async def test_adapter_funding_falls_back_on_error():
    from app.services.exchanges.adapters.delta_india import DeltaIndiaAdapter
    a = DeltaIndiaAdapter(api_key="x", api_secret="y", is_paper=True)
    a._public_get = AsyncMock(side_effect=RuntimeError("network down"))
    r = await a.get_funding_rate(27)
    assert r["source"] == "fallback"
    assert r["funding_rate_8h_pct"] == 0.0001


@pytest.mark.asyncio
async def test_adapter_l2_book_uses_product_cache():
    from app.services.exchanges.adapters.delta_india import DeltaIndiaAdapter
    a = DeltaIndiaAdapter(api_key="x", api_secret="y", is_paper=True)
    a._product_id_cache["BTCUSD"] = 27
    a._public_get = AsyncMock(return_value={"result": {
        "buy": [{"price": "49990", "size": "1"}],
        "sell": [{"price": "50010", "size": "2"}],
        "timestamp": int(time.time() * 1_000_000),
    }})
    book = await a.get_l2_book(27, depth=5)
    assert book["bids"][0] == [49_990, 1]
    assert book["asks"][0] == [50_010, 2]
