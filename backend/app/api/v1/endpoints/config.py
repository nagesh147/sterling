...
async def set_data_source(body: DataSourceRequest, request: Request) -> DataSourceResponse:
    ...
    if exchange not in _adm.SUPPORTED_DATA_SOURCES: raise HTTPException(400, f"Unsupported exchange: {exchange!r}. Supported: {list(_adm.SUPPORTED_DATA_SOURCES)}")
    try:
    ...
    except Exception as exc: raise HTTPException(502, f"Failed to connect to {exchange}: {exc}") from exc
...
async def nifty_orb_options_snapshot(user: UserContext = Depends(get_current_user)) -> dict:
    ...
    except Exception as exc: raise HTTPException(502, f"NIFTY ORB snapshot failed: {exc}") from exc
...
async def nifty_orb_options_backtest(body: dict) -> dict:
    ...
    if not isinstance(rows,list): raise HTTPException(422,"bars must be a list of OHLCV rows")
...
async def nifty_orb_options_execute(user: UserContext = Depends(get_current_user)) -> dict:
    ...
    cfg = get_config()
    if cfg.paper_only: raise HTTPException(409,"Strategy is paper-only. Disable paper_only after validation.")
    snap = await snapshot(user.user_id); plan = snap.get("plan") or {}; contract = plan.get("contract") or {}; quantity = int(plan.get("quantity") or 0)
    if not contract.get("symbol") or quantity <= 0: raise HTTPException(409,"No executable NIFTY ORB trade plan is active.")
