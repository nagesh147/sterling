...
async def reset_risk_config() -> RiskParams:
    global _risk
    _risk = RiskParams(capital=settings.default_capital, max_position_pct=settings.max_position_pct, max_contracts=settings.max_contracts, hybrid_st_weight=0.5)
    return _risk
...
async def set_data_source(body: DataSourceRequest, request: Request) -> DataSourceResponse:
    ...
    except Exception as exc: raise HTTPException(status_code=502, detail=f"Failed to connect to {exchange}: {exc}") from exc
...
async def get_telegram_config() -> TelegramConfigResponse:
    ...
    token, chat = _tg.TELEGRAM_TOKEN, _tg.TELEGRAM_CHAT_ID
...
async def set_telegram_config(body: TelegramConfigRequest) -> TelegramConfigResponse:
    import app.services.notifications.telegram as _tg
    from app.services import db as _db
    if body.bot_token.strip(): _tg.TELEGRAM_TOKEN = body.bot_token.strip()
    if body.chat_id.strip() or not _tg.TELEGRAM_CHAT_ID: _tg.TELEGRAM_CHAT_ID = body.chat_id.strip()
    _db.set_config("telegram_bot_token", _tg.TELEGRAM_TOKEN); _db.set_config("telegram_chat_id", _tg.TELEGRAM_CHAT_ID)
    reachable = False
...
async def get_circuit_breaker(request: Request) -> dict:
    ...
    if cb is None: return {"state": "ok", "halted": False, "size_multiplier": 1.0}
...
async def get_eval_history_cap() -> EvalHistoryCapResponse:
    from app.services import eval_history; return EvalHistoryCapResponse(cap=eval_history.get_cap())
@router.put("/eval-history-cap")
async def set_eval_history_cap(cap: int = 50) -> EvalHistoryCapResponse:
    from app.services import eval_history; eval_history.set_cap(cap); return EvalHistoryCapResponse(cap=eval_history.get_cap())

class NiftyOrbConfigRequest(BaseModel):
    enabled: bool | None = None; underlying: str | None = None; interval_minutes: int | None = None; opening_range_minutes: int | None = None
    entry_start: str | None = None; entry_end: str | None = None; min_breakout_atr: float | None = None; volume_multiplier: float | None = None
    vwap_slope_lookback: int | None = None; trend_lookback: int | None = None; atr_period: int | None = None; stop_buffer_atr: float | None = None
    trail_atr: float | None = None; target_r: float | None = None; option_moneyness: str | None = None; option_steps_itm: int | None = None
    max_risk_inr: float | None = None; max_trades_per_day: int | None = None; avoid_expiry_day: bool | None = None; expiry_selection: str | None = None
    execution_broker: str | None = None; data_source: str | None = None; paper_only: bool | None = None
...
async def nifty_orb_options_snapshot(user: UserContext = Depends(get_current_user)) -> dict:
    ...
    except Exception as exc: raise HTTPException(status_code=502, detail=f"NIFTY ORB snapshot failed: {exc}") from exc
...
async def nifty_orb_options_backtest(body: dict) -> dict:
    ...
    if not isinstance(rows, list): raise HTTPException(status_code=422, detail="bars must be a list of OHLCV rows")
...
async def nifty_orb_options_execute(user: UserContext = Depends(get_current_user)) -> dict:
    ...
    cfg = get_config()
    if cfg.paper_only: raise HTTPException(status_code=409, detail="Strategy is paper-only. Disable paper_only after validation.")
    snap = await snapshot(user.user_id); plan = snap.get("plan") or {}; contract = plan.get("contract") or {}; quantity = int(plan.get("quantity") or 0)
    if not contract.get("symbol") or quantity <= 0: raise HTTPException(status_code=409, detail="No executable NIFTY ORB trade plan is active.")
    return {"plan": plan, "execution": await place_manual_order(user.user_id, contract["symbol"], "BUY", quantity, exchange="NFO")}
