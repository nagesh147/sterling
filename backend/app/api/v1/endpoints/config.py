...
async def set_data_source(body: DataSourceRequest, request: Request) -> DataSourceResponse:
    ...
    if exchange not in _adm.SUPPORTED_DATA_SOURCES: raise HTTPException(status_code=400, detail=f"Unsupported exchange: {exchange!r}. Supported: {list(_adm.SUPPORTED_DATA_SOURCES)}")
    try:
...
async def test_telegram() -> TelegramConfigResponse:
    ...
    from app.services import db as _db
    reachable = False
    if _tg.TELEGRAM_TOKEN and _tg.TELEGRAM_CHAT_ID: reachable = await _tg.send("<b>Sterling test message</b>\nTelegram notifications are working.", parse_mode="HTML")
...
async def nifty_orb_options_execute(user: UserContext = Depends(get_current_user)) -> dict:
    ...
    cfg = get_config()
    if not cfg.enabled: raise HTTPException(status_code=409, detail="NIFTY ORB strategy is disabled.")
