...
async def set_telegram_config(body: TelegramConfigRequest) -> TelegramConfigResponse:
    ...
    _db.set_config("telegram_verified", "1" if reachable else "0"); _tg.TELEGRAM_REACHABLE = reachable
...
async def nifty_orb_options_execute(user: UserContext = Depends(get_current_user)) -> dict:
    ...
    cfg = get_config()
    if cfg.paper_only:
        raise HTTPException(409, "Strategy is paper-only. Disable paper_only after validation.")
    ...
    if not contract.get("symbol") or quantity <= 0:
        raise HTTPException(409, "No executable NIFTY ORB trade plan is active.")
    side = "BUY"
    result = await place_manual_order(user.user_id, contract["symbol"], side, quantity, exchange="NFO")
