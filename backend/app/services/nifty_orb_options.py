...
async def snapshot(uid:str)->dict[str,Any]:
    ...
    if cfg.data_source=='kite': bars=await _kite_bars(uid,f'{cfg.interval_minutes}m'); signal=generate_signal(bars,cfg); contracts=await _kite_options(uid,signal.direction) if signal.direction!='NONE' else []
    else:
        ...
        try: bars=[_bar(r) for r in await client.get_last_bars('NIFTY 50',240,interval=f'{cfg.interval_minutes}min')]; signal=generate_signal(bars,cfg); contracts=normalize_option_chain(await client.get_option_chain('NIFTY','nearest')) if signal.direction!='NONE' else []
