...
_TRADE_STATE_PREFIX = "nifty_orb_options_trade_state:"
...
async def _kite_options(uid: str, direction: str) -> list[OptionContract]:
    ...
    today = datetime.now(_IST).date(); wanted = "CE" if direction == "LONG" else "PE"; candidates = []
    for r in rows:
    ...
    for exp, r in candidates:
        ...
        try:
            q = await client.get_quote([f"NFO:{sym}"]); d = q.get(f"NFO:{sym}", {}) or {}; dep = d.get("depth") or {}
            buy = (dep.get("buy") or [{}])[0]; sell = (dep.get("sell") or [{}])[0]
...
async def snapshot(uid: str) -> dict[str, Any]:
    ...
    if cfg.data_source == "kite":
        bars = await _kite_bars(uid, f"{cfg.interval_minutes}m"); signal = generate_signal(bars, cfg)
        ...
        try:
            bars = [_bar(r) for r in await client.get_last_bars("NIFTY 50", 240, interval=f"{cfg.interval_minutes}min")]; signal = generate_signal(bars, cfg)
...
def _trade_state(uid: str) -> dict:
    from app.services import db
    key = _TRADE_STATE_PREFIX + uid
    try:
        raw = db.get_config(key); return json.loads(raw) if raw else {"date": "", "count": 0, "last_signal": ""}
    except Exception: return {"date": "", "count": 0, "last_signal": ""}


def _save_trade_state(uid: str, state: dict) -> None:
    from app.services import db
    db.set_config(_TRADE_STATE_PREFIX + uid, json.dumps(state, separators=(",", ":")))
...
async def execute_auto(uid: str) -> dict[str, Any]:
    """Execute one ORB entry through the universal Kite execution/safety path.

    This function owns strategy-level eligibility only. Paper/Live is owned by the
    active Kite account/client and Manual/Auto is owned by the universal engine config.
    """
    cfg = get_config()
    ...
    if cfg.avoid_expiry_day and datetime.now(_IST).date().isoformat() == "": return {"status": "blocked", "reason": "expiry-day trading disabled"}
    state = _today_trade_state(uid)
    if int(state["count"]) >= cfg.max_trades_per_day: return {"status": "daily_limit", "count": state["count"]}
    if any(p.status in (positions.OPEN, positions.PENDING) and str(p.underlying).upper() in {"NIFTY", "NIFTY 50"} for p in positions.open_positions(uid)):
    ...
    if state.get("last_signal") == signal_key: return {"status": "duplicate_signal"}
    ...
    qty = int(plan["quantity"]); symbol = str(contract["symbol"]); side = "BUY"
    idem = live_safety.make_idempotency_key(uid, symbol, side, qty, int(datetime.now(_IST).timestamp() * 1000))
    ...
    client = await ka.acquire_client(acct)
    # Resolve the broker token because TrueData contracts do not carry a Kite token.
    rows = await client.search_instruments("NIFTY", "NFO", limit=5000)
    inst = next((r for r in rows if str(r.get("tradingsymbol") or "").upper() == symbol.upper()), None)
    if not inst: return {"status": "blocked", "reason": f"Kite contract {symbol} is unavailable"}
    try:
        quote = await client.get_quote([f"NFO:{symbol}"]); q = (quote or {}).get(f"NFO:{symbol}", {}) or {}
    ...
    state["count"] = int(state["count"]) + 1; state["last_signal"] = signal_key; _save_trade_state(uid, state)
...
async def execute_manual(uid: str) -> dict[str, Any]:
    """Manual ORB execution uses the same plan and protection path as auto execution."""
    cfg = get_config()
