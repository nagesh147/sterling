...
def get_config() -> StrategyConfig:
    try:
        ...
        if raw:
            ...
            return StrategyConfig(**{**_default().__dict__, **data})
    except Exception:
        pass
    return _default()
...
def normalize_truedata_option_chain(payload: Any, expiry: str | None = None) -> list[OptionContract]:
    if isinstance(payload, dict): rows = payload.get("Records") or payload.get("records") or payload.get("data") or payload.get("options") or []
    else: rows = payload
    if isinstance(rows, dict): rows = rows.get("Records") or rows.get("data") or []
    ...
    for r in rows or []:
        ...
        try: strike = float(r.get("strike") or r.get("strike_price")); ltp = float(r.get("ltp") or r.get("last_price") or r.get("close") or 0)
        except (TypeError, ValueError): continue
        out.append(OptionContract(symbol=symbol, strike=strike, expiry=exp, option_type=typ, ltp=ltp, bid=float(r.get("bid") or r.get("bid_price") or 0), ask=float(r.get("ask") or r.get("ask_price") or 0), lot_size=int(r.get("lot_size") or r.get("lotsize") or 75), delta=float(r["delta"]) if r.get("delta") not in (None, "") else None, volume=float(r.get("volume") or 0), open_interest=float(r.get("oi") or r.get("open_interest") or 0)))
    return out
...
async def _kite_spot_and_options(user_id: str, direction: str) -> tuple[float, list[OptionContract]]:
    ...
    for r in instruments:
        ...
        try: exp_date = datetime.strptime(expiry, "%Y-%m-%d").date()
        except ValueError: continue
        if exp_date < today: continue
        if (typ == "CE" and direction == "LONG") or (typ == "PE" and direction == "SHORT"): candidates.append((exp_date, r))
    if not candidates: raise RuntimeError("No NIFTY option contracts available")
    nearest = min(x[0] for x in candidates); contracts: list[OptionContract] = []
    for exp_date, r in candidates:
        if exp_date != nearest: continue
        symbol = str(r.get("tradingsymbol") or "")
        try:
            q = await client.get_quote([f"NFO:{symbol}"]); data = q.get(f"NFO:{symbol}", {}) or {}; depth = data.get("depth") or {}; buy = (depth.get("buy") or [{}])[0]; sell = (depth.get("sell") or [{}])[0]
            contracts.append(OptionContract(symbol=symbol, strike=float(r.get("strike") or 0), expiry=exp_date.isoformat(), option_type=str(r.get("instrument_type")), ltp=float(data.get("last_price") or 0), bid=float(buy.get("price") or 0), ask=float(sell.get("price") or 0), lot_size=int(r.get("lot_size") or 1), volume=float(data.get("volume") or 0), open_interest=float(data.get("oi") or 0)))
...
async def snapshot(user_id: str) -> dict[str, Any]:
    ...
    if cfg.data_source == "kite":
        bars = await _kite_bars(user_id, f"{cfg.interval_minutes}m"); signal = generate_signal(bars, cfg); spot = bars[-1].close; contracts: list[OptionContract] = []
        if signal.direction != "NONE": spot, contracts = await _kite_spot_and_options(user_id, signal.direction)
    else:
        ...
        try:
            bars_raw = await client.get_last_bars("NIFTY 50", 200, interval=f"{cfg.interval_minutes}min"); bars = [_bar_from_any(r) for r in bars_raw]; signal = generate_signal(bars, cfg); spot = bars[-1].close; contracts = []
            if signal.direction != "NONE": contracts = normalize_truedata_option_chain(await client.get_option_chain("NIFTY", "nearest"))
...
def backtest_from_bars(rows: list[dict[str, Any]], cfg: StrategyConfig | None = None) -> dict[str, Any]:
    cfg = cfg or get_config(); bars = [_bar_from_any(r) for r in rows]
    if len(bars) < 100: return {"metrics": summarize_pnl([]), "warning": "At least 100 bars are required"}
    pnls: list[float] = []; current_day = None; day_trades = 0
    for i in range(60, len(bars)):
        prefix = bars[:i + 1]; day = prefix[-1].timestamp.date()
        ...
        entry = prefix[-1].close; stop = entry - signal.atr * cfg.stop_buffer_atr if signal.direction == "LONG" else entry + signal.atr * cfg.stop_buffer_atr; target = entry + signal.atr * cfg.target_r if signal.direction == "LONG" else entry - signal.atr * cfg.target_r; outcome = None
        for b in bars[i + 1:]:
            ...
            if signal.direction == "LONG":
                ...
                if b.low <= target: outcome = target - entry if False else entry - target; break
        if outcome is not None: pnls.append(outcome); day_trades += 1
    metrics = summarize_pnl(pnls); metrics["model"] = "underlying-point baseline"; metrics["costs_included"] = False
