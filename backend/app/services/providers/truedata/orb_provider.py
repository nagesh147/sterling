...
class TrueDataOrbProvider:
    ...
    def _parse_provider_time(raw:Any)->datetime:
        text=str(raw or '').strip().replace('Z','+00:00');dt=datetime.fromisoformat(text) if 'T' in text or '+' in text[10:] else datetime.strptime(text,'%Y-%m-%d %H:%M:%S');return (dt.replace(tzinfo=IST) if dt.tzinfo is None else dt).astimezone(timezone.utc)
    ...
    async def latest_tick(self,symbol:str,*,bidask:bool=True)->dict[str,Any]|None:
        rows=await self.client.get_last_ticks(symbol,1,bidask=1 if bidask else 0);return rows[-1] if rows else None
    @staticmethod
    ...
    async def resolve_expiry(self,symbol:str,cfg:StrategyConfig)->str:
        payload=await self.client.get_all_symbols('NFO',search=symbol,allexpiry=True);expiries=self._extract_expiries(payload);eligible=[d for d in expiries if cfg.expiry_dte_min<=(d-date.today()).days<=cfg.expiry_dte_max]
    ...
    async def option_chain(self,symbol:str,expiry:str,cfg:StrategyConfig)->list[OptionContract]:
        ...
        # Do not hydrate every contract with a tick. Chain discovery is cheap; the
        # scanner refreshes only the selected candidate immediately before planning.
        return filter_chain(normalize_chain(payload),cfg,today=date.today())
    async def refresh_contract(self,contract:OptionContract,cfg:StrategyConfig)->tuple[OptionContract,float|None]:
        if not cfg.truedata_use_ticks:return contract,None
        tick=await self.latest_tick(contract.symbol,bidask=cfg.truedata_use_bid_ask)
        if not tick:raise ValueError('TrueData returned no latest tick for selected option')
        stamp=self._tick_time(tick)
        age=None if stamp is None else max(0.0,(datetime.now(timezone.utc)-stamp).total_seconds())
        if cfg.truedata_use_quote_freshness and (age is None or age>cfg.max_quote_staleness_s):raise ValueError(f'TrueData quote is stale: {age if age is not None else "unknown"}s')
    ...
    async def symbols(self,segment:str='NFO',search:str|None=None)->Any:return await self.client.get_all_symbols(segment,search=search,allexpiry=False)
