...
from app.engines.orb_momentum_options import ORBMomentumConfig, UnderlyingBar, ORBTradeSignal, generate_signal, select_option, build_trade_signal, OptionCandidate
...
class ORBMomentumScanner:
    ...
    def evaluate(self, symbol: str, bars: Sequence[UnderlyingBar], contracts: Sequence[OptionCandidate]) -> dict | None:
        ...
        if self.state.session_date != today:
            self.state = ScannerState([], 0, 0, today)
        ...
        payload.update({"status": "SIGNAL", "strategy": "ORB_MOMENTUM_OPTIONS", "data_source": self.config.data_source})
        self.state.signals.append(payload); self.state.signals_today += 1
        return payload

    def signals(self) -> list[dict]:
        return list(reversed(self.state.signals))
