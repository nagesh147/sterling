...
async def scan_option_candidates(
    ...
    signals=await scan_universe(instruments,strategy_config=strategy_config,scan_config=scan_config,fetch_bars=fetch_bars)
    ...
    async def resolve(candidate: UniverseSignal)->OptionSignalCandidate|None:
        async with semaphore:
            try:
                ...
                contracts=filter_chain(contracts,strategy_config)
                if not contracts:return None
                planned=plan_signal(candidate,contracts,strategy_config)
