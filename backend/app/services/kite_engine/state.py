...
def activity(uid: str, limit: int = 200) -> List[ActivityEvent]:
    ...
    return [] if not buf else list(buf)[-limit:]
...
def clear_cooldown(uid: str) -> bool:
    s = status(uid)
    if s.cancel_cooldown_ms and time.time() * 1000 < s.cancel_cooldown_ms:return True
    return False
...
def _persist_daily_pnl(uid: str) -> None:
    ...
    db.set_config(f"kite_engine_daily_pnl_{uid}",json.dumps([cur[0],cur[1]]) if cur else "")

def record_realized_pnl(uid: str, pnl: float, *, day_iso: str | None = None) -> float:
    day=day_iso or _ist_today_iso();cur=_load_daily_pnl(uid);total=(cur[1]+float(pnl)) if cur and cur[0]==day else float(pnl)
    _daily_pnl[uid]=(day,total)
    try:_persist_daily_pnl(uid)
    except Exception as exc:
        # Do not claim durable risk state after a persistence failure. The caller can
        # close/reconcile, while subsequent safety checks fail closed on strict reads.
...
def daily_realized_pnl_strict(uid: str, *, day_iso: str | None = None) -> float:
    """Broker-risk read: database failures raise instead of becoming zero."""
    day=day_iso or _ist_today_iso();cur=_load_daily_pnl(uid);return cur[1] if cur and cur[0]==day else 0.0
...
def correlation_penalty(uid: str, new_asset: str, open_assets: list) -> float:
    ...
    try:return trk.size_multiplier(new_asset,open_assets)
    except Exception:return 1.0

# Keep the remaining public state API imported by existing Kite modules.
def clear_auto_open_all(uid: str) -> None:
    _auto_open[uid]=set();_persist_auto_open(uid)
