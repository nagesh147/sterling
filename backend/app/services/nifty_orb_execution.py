...
async def _existing_order_by_tag(client, tag: str) -> dict | None:
    """Durable idempotency: broker order tags survive process restarts."""
    try:
...
async def _resolve_fill(client, order_id: str, *, timeout_s: float = 3.0) -> tuple[int, float, str]:
    """Reconcile broker state before protection is armed.

    Returns (filled_quantity, average_price, status). Partial fills are real exposure;
    the protection registry must never be armed for the requested quantity.
    """
...
async def execute_scan(uid: str, *, scan: dict[str, Any], max_trades: int) -> dict[str, Any]:
    ...
    for row in scan.get("signals", []):
        ...
        # Deterministic across process restarts. The broker tag is also searched below,
        # making idempotency durable rather than merely an in-memory 60-second guard.
        ...
        if existing:
            ...
            try:
                ...
                if entry <= 0:
                    continue
                order = await client.place_order_option(symbol, "buy", requested_quantity, exchange=exchange, tag=idem)
            except Exception as exc:
                executed.append({"status": "error", "underlying": underlying, "symbol": symbol, "error": str(exc)})
                continue
            order_id = str((order or {}).get("order_id") or "")
            if not order_id:
                executed.append({"status": "error", "underlying": underlying, "symbol": symbol, "error": "Broker returned no order id"})
                continue
        ...
        if filled_qty <= 0:
            if broker_status not in {"CANCELLED", "REJECTED"}:
                await _cancel_unfilled_remainder(client, order_id)
            ...
            continue
        if filled_qty < requested_quantity:
            await _cancel_unfilled_remainder(client, order_id)
        ...
        try:
            armed = await protection.arm_position(
                ...
                entry_premium=fill_price or float(plan.get("entry_premium") or 0),
        ...
        # A filled order consumes a trade slot even if protection failed. This prevents
        # a protection error from opening additional exposure under the daily cap.
