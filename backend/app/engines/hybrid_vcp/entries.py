...
class EntryGate:
    ...
    mode:            VolMode
    regime:           Optional[Regime]  = None  # detected market regime
    entry_price:     Optional[float]  = None  # next-bar open, set by backtest
...
class EntryConfig:
    ...
    high_vol_risk_red: float = 1.0  # risk multiplier in HIGH_VOL regime
...
def evaluate_gate(
    ...
    # Per-bar regime (multi-factor: ATR ratio, range, volume, price position)
    regime = Regime.NEUTRAL
    if idx >= 30:
        regime = detect_regime(closes, highs, lows, bundle.atr, volume, idx, lookback=60)

    # ── 0. Regime filter — hard no-trade in CHOP ─────────────────
    if regime == Regime.CHOP:
        return EntryGate(False, Direction.NONE, f"chop_regime", 0.0, None, mode, regime, None)
    ...
    if raw_dir == 0:
        return EntryGate(False, Direction.NONE, "no_signal", 0.0, None, mode, regime, None)
    ...
    if mode == VolMode.COMPRESSION:
        ...
        if raw_dir == 1:   # long
            if not (cfg.min_ibs_long <= ib <= cfg.max_ibs_long):
                return EntryGate(False, direction, f"ibs_oob({ib:.2f})", 0.0, None, mode, regime, None)
            if not (cfg.min_rsi_long <= rs <= cfg.max_rsi_long):
                return EntryGate(False, direction, f"rsi_oob({rs:.1f})", 0.0, None, mode, regime, None)
        else:              # short
    ...
    if micro.has_divergence:
        return EntryGate(False, direction, f"divergence", 0.0, micro, mode, regime, None)

    if micro.flow_score < cfg.flow_threshold:
        return EntryGate(False, direction, f"flow_low({micro.flow_score:.2f})", 0.0, micro, mode, regime, None)
