...
def build_snapshot_signals(
    *,
    ...
    for raw in chosen:
        ...
        current_spot = spot_exit or _num(session.get("exit_fill_price")) or _num(session.get("last_vwap")) or spot_entry
        cached = list((live.get(underlying_for(tape)) or {}).get("legs") or [])
        option_legs = (
