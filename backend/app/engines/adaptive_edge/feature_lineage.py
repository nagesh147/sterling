...
class FeatureDefinition:
    ...
    def __post_init__(self) -> None:
        for name in ("feature_id", "feature_definition_version", "transformation_version", "source_dataset_version", "unit", "semantic_definition"):
...
class FeatureInput:
    ...
    def __post_init__(self) -> None:
        ...
        if self.availability_time < self.observation_time:
            raise FeatureLineageError(f"availability_time precedes observation_time for {self.name}")
...
class FeatureSnapshot:
    ...
    def __post_init__(self) -> None:
        ...
        for name, available_at in self.available_at.items():
            ...
            if available_at > self.decision_time:
                raise FeatureLineageError(f"lookahead detected for feature {name}: {available_at.isoformat()} > {self.decision_time.isoformat()}")
    ...
    def assert_causal(self, decision_time: datetime | None = None) -> None:
        ...
        for name, available_at in self.available_at.items():
            if available_at > boundary:
                raise FeatureLineageError(f"lookahead detected for feature {name}: {available_at.isoformat()} > {boundary.isoformat()}")
    ...
    def provenance_for(self, feature_id: str) -> FeatureProvenance:
        ...
        if len(matches) != 1:
            raise FeatureLineageError(f"expected exactly one provenance record for {feature_id}, got {len(matches)}")
        return matches[0]
...
class FeatureDependencyGraph:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "dependencies", MappingProxyType({key: tuple(value) for key, value in self.dependencies.items()}))
        self.assert_acyclic()
...
def build_feature_snapshot(*, snapshot_id: str, decision_time: datetime, inputs: Sequence[FeatureInput], definitions: Mapping[str, FeatureDefinition], formula_ids: Sequence[str] = (), quality_state: FeatureQuality = FeatureQuality.AVAILABLE, source_versions: Sequence[str] = (), transformation_versions: Sequence[str] = ()) -> FeatureSnapshot:
    ...
    if watermark > decision_time:
        raise FeatureLineageError(f"feature availability watermark exceeds decision time: {watermark.isoformat()} > {decision_time.isoformat()}")
    values = {item.name: item.value for item in inputs}
    available_at = {item.name: item.availability_time for item in inputs}
    dataset_versions = tuple(dict.fromkeys([*source_versions] + [version for item in inputs for version in item.source.source_dataset_versions]))
    transformation = tuple(dict.fromkeys(transformation_versions))
    provenance = tuple(FeatureProvenance(feature_id=definitions[item.name].feature_id, feature_version=definitions[item.name].feature_definition_version, snapshot_id=snapshot_id, decision_time=decision_time, source_event_ids=tuple(item.source.source_event_ids), source_dataset_versions=tuple(item.source.source_dataset_versions), transformation_version=definitions[item.name].transformation_version, availability_watermark=item.availability_time, quality_state=item.quality) for item in inputs)
    snapshot = FeatureSnapshot(snapshot_id=snapshot_id, observation_time=max(item.observation_time for item in inputs), decision_time=decision_time, values=values, available_at=available_at, definitions=definitions, provenance=provenance, source_versions=dataset_versions, transformation_versions=transformation, availability_watermark=watermark, quality_state=quality_state, formula_ids=tuple(formula_ids))
    snapshot.assert_causal()
    return snapshot


def build_causal_rolling_window(observations: Sequence[FeatureInput], *, decision_time: datetime, lookback_seconds: int, include_boundary: bool = True) -> tuple[FeatureInput, ...]:
    _require_aware(decision_time, "decision time")
...
def reconstruct_model_state(states: Sequence[tuple[datetime, object]], *, decision_time: datetime) -> object | None:
    _require_aware(decision_time, "decision time")
    prior = []
    for timestamp, state in states:
        _require_aware(timestamp, "state time")
