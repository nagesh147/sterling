...
def run_e2e(event: CanonicalMarketEvent, *, feature_builder: FeatureBuilder, prediction_engine: PredictionEngine, edge_formula: EdgeFormula, decision_engine: DecisionEngine, risk_authorizer: RiskAuthorizer, instrument_selector: InstrumentSelector, order_factory: OrderIntentFactory, execution_gateway: ExecutionGateway, position_projector: PositionProjector, lifecycle_engine: LifecycleEngine, execution_cost: float, minimum_net_value: float = 0.0, required_formula_ids: tuple[str, ...] | None = None, broker_event: BrokerExecutionEvent | None = None) -> E2ETrace:
    """Run one candidate through every authorized layer.

    A simulation/research scope must be supplied both to this runner and to
    ExecutionGateway.authorized_formula_ids. Production uses the default full
    required formula set and therefore remains fail-closed while any formula is locked.
    """
    audit = AuditLedger()
    ...
    formula_ids = REQUIRED_STRATEGY_FORMULAS if required_formula_ids is None else tuple(required_formula_ids)
    ...
    return E2ETrace(event, snapshot, prediction, edge, economics, decision, authorization, instrument, order, execution, position, lifecycle, audit.records(), gate)
