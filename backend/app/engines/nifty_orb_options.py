...
def select_option(spot: float, direction: Direction, contracts: Sequence[OptionContract], cfg: StrategyConfig) -> OptionContract:
    ...
    if not candidates:
        raise ValueError(f"No liquid {typ} contracts satisfy the configured liquidity gates")
...
def build_trade_plan(signal: Signal, option: OptionContract, cfg: StrategyConfig, *, spot: float) -> TradePlan:
    ...
    return TradePlan(
        signal.direction, option.option_type, option, spot, stop, risk_points,
        abs(target - spot), entry_premium, stop_premium, target_premium,
        premium_risk_per_share, quantity, risk, signal.reason)
...
def summarize_pnl(pnls: Iterable[float]) -> dict:
    ...
    return {
        ...
        "expectancy": expectancy, "average_win": mean(wins) if wins else 0.0,
        "average_loss": mean(losses) if losses else 0.0, "max_drawdown": max_dd,
        "net_pnl": sum(values),
    }
