...
def select_option(underlying:float,direction:Direction,contracts:Sequence[OptionCandidate],cfg:ORBMomentumConfig)->OptionCandidate|None:
    ...
    # Hard BUY-only invariant: the scanner can only select a long option contract.
    pool=[c for c in contracts if c.option_type==wanted and c.ltp>0 and c.ask>0 and c.lot_size>0]
    ...
    return min(pool,key=lambda c:abs(c.strike-underlying))
...
def build_trade_signal(signal:ORBSignal,option:OptionCandidate,cfg:ORBMomentumConfig)->ORBTradeSignal:
    ...
    entry=option.ask if option.ask>0 else option.ltp
    risk_per_unit=max(signal.atr*0.10,entry*0.15)
    lot=option.lot_size
    lots=max(1,int(cfg.max_risk_inr/(risk_per_unit*lot)))
    quantity=lots*lot
