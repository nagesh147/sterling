...
| 26 | No way to tell a customised value from a default | Each setting is marked `default` or `changed · default X ↺` with one-click restore, and a moved field is tinted. **The defaults come from the engine** (`GET` now returns `defaults`) rather than a client copy that could drift. See 27-29 for the field types this first pass missed and the counts it got wrong. |

| 27 | Default indication covered only the numeric fields | Chart source, moneyness, expiry preference, the expiry-day rule, the entry window and the four TrueData switches carried no default marker at all — roughly a third of the panel, and the third a user is most likely to touch first. The badge is now a shared `DefaultBadge` every field type uses, and a switch reads `default skipped`, not `default true`. |
| 28 | `Advanced · N` meant something different here than everywhere else | Navigator and Trade Rules pass the number of settings the section holds. ORB passed the number that differed from default, so one badge meant two things in one settings hub. ORB now counts its settings; the changed count lives in each section summary and in a panel-wide note above the sections. |
| 29 | A section summary reported another section's edits | "Signal thresholds" was reading the panel-wide changed count, so moving the chart source made it claim a threshold had moved. Each summary now counts only the keys its own section owns. |
| 30 | The settings panel had no tests | The default indication was verified only by driving the live app. Eleven tests now pin it: the at-default state, a badged numeric/choice/switch, the entry window restoring both ends as one setting, per-section counts, that the power switch is not counted as a setting, patch-only saves, and that reset preserves whether the engine is running. |
| 31 | Gamma, theta and vega were printed but untested | Only price, delta and implied volatility were pinned; `black_scholes_greeks` and `implied_greeks` had no direct coverage while the board displayed their output. Now pinned against textbook values *and* against a finite-difference reprice and one-day decay — which is what actually catches the units being wrong (vega per volatility point, theta per calendar day). |
| 32 | Nothing pinned the published defaults | The whole default indication rests on `GET /config/nifty-orb-options` returning `defaults`. A test now asserts it equals the engine's own `StrategyConfig()` field for field, and that those published defaults are themselves valid — reset must not hand the engine a config it would reject. |
