...
| E17 | **The stale-tick arrival mechanism is captured, not inferred.** A live binary tick pulled through `ticker.parse_packet` (market closed, 2026-08-21 23:16 IST) for the traded PE reports `last_price 358.65`, `last_trade_time 1787306999` (15:39:59 IST) and `exchange_timestamp 1787309751` (16:25:51 IST) — the two clocks **2752 s apart in one tick**. So a subscription taken before a session's first trade yields a price stamped in a previous session, while the packet clock keeps advancing | live Kite tick capture | CONFIRMED (external) |
| E18 | `ohlc.open` is **session-to-date and carries no timestamp**: the same capture reports `ohlc.open = 356.70`, which is *Friday's* 09:15 open (independently equal to the 2026-08-21 09:15 bar open), not the next session's | live Kite tick capture | CONFIRMED (external) |
| E19 | With the market closed the L1 book is empty (`bid/ask = 0.0`), so an ask-referenced entry policy refuses rather than guessing | live Kite tick capture | CONFIRMED (external) |
