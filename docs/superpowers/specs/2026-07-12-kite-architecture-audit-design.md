...
1. **`db_postgres.py` hardcoded credentials, live in `main.py:1345`** — unconditionally invoked with `"password"`/`127.0.0.1` baked in. Fix: source from env, and gate behind the same `settings.use_sqlalchemy`-style opt-in as the rest of the Postgres path, or remove if unused.
2. **`get_current_user()` trusts a spoofable `X-User-Id` header** (`auth.py:27`), defaulting to `"default"`, sitting in front of per-user encrypted Kite credentials. Fix: real server-issued session identity (signed cookie/JWT) before any further multi-account rollout.
3. **No alembic migrations** despite a live SQLAlchemy dual-write path (`db.py:286-321`) — schema drift risk on the next change. Add migrations before touching that path again.
4. Confirm Fernet key storage/rotation for encrypted Kite tokens (`security.py`) and confirm no `verify=False` anywhere in the Kite `httpx` client TLS config.
