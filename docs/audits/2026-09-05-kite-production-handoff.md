...
Concrete release blockers: incomplete transactional fill/PnL accounting; ambiguous entry submission recovery; unverified current executable contract data; incomplete quote freshness, capital reservation and mandatory portfolio risk gates. The connected lake's registered removable-drive path was unavailable during inspection. The available index cache ends **2026-07-16**, before CAS commenced.
...
| Existing execution/scanner test fixtures | Historic execution fixtures isolate the new entry-data dependency; positive lows; explicit confirmed-fill events; removed unsafe one-lot/ACK-close/GTT-race expectations | These plumbing fixtures deliberately bypass the live session gate; boundary tests exercise that gate separately. Fixtures are not market-performance evidence. |
...
|---|---|---|
| P0 durable entry intents | New `backend/app/services/kite_engine/order_journal.py`; versioned migration; `service.py`, `positions.py`, `monitor.py` | Atomically reserve `(account_id, strategy_id, generation_id, signal_id, contract_id, side)`, persist an alphanumeric client tag before sending, then attach ACK/order IDs. Timeout must leave ENTRY_UNKNOWN. Recover via broker orders/trades; absent/ambiguous evidence blocks resend. Crash immediately before/after send, then restart two workers: at most one attributable order. |
