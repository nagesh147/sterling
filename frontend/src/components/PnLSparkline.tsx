...
export function PnLSparkline({ positionId, entrySpot }: Props) {
  ...
  if (!data || data.count < 2) {
    return (
      <div style={{ color: 'var(--k-text)', fontSize: 10 }}>
  ...
  const lineColor = lastPnl >= 0 ? 'var(--k-green-mint)' : 'var(--k-red-muted)';
  const path = buildPath(pnls, min, range);

  return (
    <div title={`P&L history (${data.count} snapshots) | Latest: ${lastPnl >= 0 ? '+' : ''}${lastPnl.toFixed(2)}`}>
      <div style={{ color: lastPnl >= 0 ? 'var(--k-green-mint)' : 'var(--k-red-muted)', fontSize: 10, marginBottom: 2 }}>
        P&L {lastPnl >= 0 ? '+' : ''}{lastPnl.toFixed(2)}
      </div>
      <svg width={W} height={H} style={{ display: 'block' }}>
        {/* Zero line */}
        <line x1={PAD} y1={zeroY} x2={W - PAD} y2={zeroY}
          stroke="var(--k-ink-1)" strokeWidth="0.5" strokeDasharray="2,2" />
        {/* P&L line */}
