...
export function SuperTrendEnginePanel() {
  ...
  return <>
    ...
    <PanelCard>
      ...
      <Section title="Trail tightness" description="Which line the stop follows once a trade is running." summary={`${trailLabel}${cfg.exit_aligned_trail?' · anchored to exit counter':''}`} defaultOpen persistKey="st-trail"><Field label={FIELDS.trail_target.label} hint={FIELDS.trail_target.help}><ChoiceRow value={cfg.trail_target} options={TRAIL_OPTIONS} onChange={v=>patch({trail_target:v})}/></Field><Field label={FIELDS.exit_aligned_trail.label} hint={FIELDS.exit_aligned_trail.help}><div style={{display:'flex',alignItems:'center',gap:9}}><Switch checked={cfg.exit_aligned_trail??false} label="Anchor stop to exit counter" onChange={()=>patch({exit_aligned_trail:!(cfg.exit_aligned_trail??false)})}/><span style={{color:TEXT,fontSize:11.5}}>{cfg.exit_aligned_trail?'Aligned to exit counter':'Tightest fast line'}</span></div></Field></Section>
      <Section title="Exit rule" description="What closes a SuperTrend trade." summary={`${exitModeLabel(cfg.exit_mode)}${(cfg.price_stop_exit??true)?' · trail enforced':' · counter only'}`} defaultOpen persistKey="st-exit"><Field label={FIELDS.exit_mode.label} hint={FIELDS.exit_mode.help}><ChoiceRow value={cfg.exit_mode} options={EXIT_MODE_OPTIONS} onChange={v=>patch({exit_mode:v})}/></Field><Field label={FIELDS.price_stop_exit.label} hint={FIELDS.price_stop_exit.help}><div style={{display:'flex',alignItems:'center',gap:9}}><Switch checked={cfg.price_stop_exit??true} label="Enforce the trailing stop as a real exit" onChange={()=>patch({price_stop_exit:!(cfg.price_stop_exit??true)})}/><span style={{color:TEXT,fontSize:11.5}}>{(cfg.price_stop_exit??true)?'Trail or exit counter, whichever fires first':'Exit counter only'}</span></div></Field><ConfigNote>Entry is fixed: all three SuperTrend lines must be green and the signal fresh on the latest closed 1H bar. Filters that can refuse an automatic entry live under <b>Automatic rules</b>. A position already held by the server-side tick monitor has its trail enforced regardless of the board exit rule.</ConfigNote></Section>
    </PanelCard>
    <div style={{marginTop:18,paddingTop:18,borderTop:`1px solid ${BORDER}`}}><div style={{marginBottom:10,color:TEXT,fontSize:13,fontWeight:750}}>NIFTY ORB + VWAP</div><div style={{marginBottom:10,color:MUTED,fontSize:11,lineHeight:1.5}}>Independent BUY-only directional options engine. It generates from NIFTY 50 and maps LONG → CE / SHORT → PE. It does not inherit SuperTrend settings.</div><NiftyOrbOptionsSettings/><div style={{marginTop:18,paddingTop:18,borderTop:`1px solid ${BORDER}`}}><NiftyOrbSignalsPanel/></div></div>
