...
export function SuperTrendEnginePanel() {
  ...
  const handleApply = () => {
    ...
    setCfg.mutate(draft, {
      ...
      onError: (err) => {
        notifyOrder({ kind: 'error', title: 'Settings NOT saved', message: `SuperTrend settings were not applied: ${String((err as Error)?.message ?? 'the save was rejected')}. Your changes are still here — try Apply again.` });
      },
    });
  };
  const handleDiscard = () => {
  ...
  const handleReset = () => {
    ...
    resetCfg.mutate(undefined, {
      onSuccess: () => { setDirty(false); setResetConfirm(false); runScan.mutate(); },
    });
  };
      ...
      <PanelCard>
        ...
        <Section title="Trail tightness" description="Which line the stop follows once a trade is running." summary={`${trailLabel}${cfg.exit_aligned_trail ? ' · anchored to exit counter' : ''}`} defaultOpen persistKey="st-trail">
          ...
          <Field label={FIELDS.exit_aligned_trail.label} hint={FIELDS.exit_aligned_trail.help}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}><Switch checked={cfg.exit_aligned_trail ?? false} label="Anchor stop to exit counter" onChange={() => patch({ exit_aligned_trail: !(cfg.exit_aligned_trail ?? false) })} /><span style={{ color: TEXT, fontSize: 11.5 }}>{cfg.exit_aligned_trail ? 'Aligned to exit counter' : 'Tightest fast line'}</span></div>
          </Field>
        </Section>
