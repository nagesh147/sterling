...
function ProductPill({ product }: { product: string }) {
  const style = product === 'NRML'
    ...
    : product === 'MIS'
      ...
      : { background: '#f1f1f1', color: C.muted };
...
function ConvertInline({ position, onClose }: { position: any; onClose: () => void }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 5, alignItems: 'center', justifyContent: 'flex-end' }} onClick={(e) => e.stopPropagation()}>
      ...
      <button type="button" disabled={invalid || convert.isPending} onClick={() => convert.mutate({ tradingsymbol: position.tradingsymbol, exchange: position.exchange, transaction_type: num(position.quantity) > 0 ? 'BUY' : 'SELL', position_type: 'day', quantity: qty, old_product: position.product, new_product: target })} style={{ border: 0, background: 'transparent', color: invalid ? '#bbb' : C.blue, fontSize: 11, cursor: invalid ? 'not-allowed' : 'pointer' }}>{convert.isPending ? '…' : 'convert'}</button>
...
function PositionInfo({ position, onClose }: { position: any; onClose: () => void }) {
  ...
  return (
    <>
      ...
      <div style={{ position: 'fixed', top: 90, left: '50%', transform: 'translateX(-50%)', width: 430, maxWidth: '92vw', background: '#fff', boxShadow: '0 10px 35px rgba(0,0,0,.25)', zIndex: 1201, borderRadius: 4 }}>
      ...
      <div style={{ maxWidth: 1120, margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, marginBottom: 14, flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 11, flexWrap: 'wrap' }}>
            {selected.size > 0 && <button disabled={exiting} onClick={exitSelected} style={{ border: 0, borderRadius: 3, background: C.red, color: '#fff', padding: '5px 10px', fontSize: 11, cursor: exiting ? 'wait' : 'pointer' }}>{exiting ? 'Exiting…' : `Exit Selected (${selected.size})`}</button>}
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', minWidth: 790, borderCollapse: 'collapse', tableLayout: 'fixed' }}>
            ...
            <thead><tr><th style={{ ...th, textAlign: 'center' }}><input type="checkbox" aria-label="Select all open positions" checked={allSelected} onChange={toggleAll} disabled={!selectable.length} /></th><th style={th}>Product</th><th style={th}>Instrument</th><th style={{ ...th, textAlign: 'right' }}>Qty.</th><th style={{ ...th, textAlign: 'right' }}>Avg.</th><th style={{ ...th, textAlign: 'right' }}>LTP</th><th style={{ ...th, textAlign: 'right', background: '#fbfbfb' }}>P&amp;L</th><th style={{ ...th, textAlign: 'right' }}>Chg.</th><th style={th} /></tr></thead>
            <tbody>{filtered.map((p: any) => {
              ...
              return <tr key={k} className="pos-row" style={{ background: selectedRow ? 'rgba(56,126,209,.05)' : undefined, opacity: open ? 1 : .82 }}>
                ...
                <td style={{ ...td, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}><InstrumentLabel symbol={p.tradingsymbol} /><span style={{ marginLeft: 6, fontSize: 9, color: C.muted, background: '#f1f1f1', padding: '1px 4px', borderRadius: 2 }}>{p.exchange}</span>{!open && <span style={{ marginLeft: 6, fontSize: 9, color: C.muted }}>CLOSED</span>}</td>
                ...
                <td style={{ ...td, textAlign: 'right', color: color(num(p.pnl)), background: '#fbfbfb' }}>{money(num(p.pnl))}</td>
            ...
            <tfoot><tr><td colSpan={6} style={{ ...td, borderTop: `1px solid ${C.border}`, borderBottom: 0, textAlign: 'right' }}>Total P&amp;L</td><td style={{ ...td, borderTop: `1px solid ${C.border}`, borderBottom: 0, textAlign: 'right', color: color(totalPnl), background: '#fbfbfb', fontSize: 14 }}>{money(totalPnl)}</td><td colSpan={2} /></tr></tfoot>
          </table>
        </div>
