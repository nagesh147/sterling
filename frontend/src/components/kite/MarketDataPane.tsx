...
const S = {
  card: { background: t.raised, border: `1px solid ${t.border}`, borderRadius: 10, padding: 14, marginBottom: 14 } as React.CSSProperties,
  title: { color: t.dim, fontSize: 11, letterSpacing: 2, marginBottom: 10, fontWeight: 700 } as React.CSSProperties,
  search: { background: t.bg, color: t.bright, border: `1px solid ${t.border}`, borderRadius: 8, padding: '10px 12px', fontFamily: 'inherit', fontSize: 14, width: '100%', boxSizing: 'border-box' as const } as React.CSSProperties,
  hint: { color: t.dim, fontSize: 11 } as React.CSSProperties,
  th: { textAlign: 'left' as const, color: t.dim, fontSize: 10, fontWeight: 600, padding: '4px 8px', borderBottom: `1px solid ${t.border}` } as React.CSSProperties,
  td: { padding: '6px 8px', fontSize: 12, color: t.bright, borderBottom: `1px solid ${tint(t.border, 50)}` } as React.CSSProperties,
  input: { background: t.bg, color: t.bright, border: `1px solid ${t.border}`, borderRadius: 6, padding: '6px 9px', fontFamily: 'inherit', fontSize: 12, boxSizing: 'border-box' as const } as React.CSSProperties,
  label: { color: t.dim, fontSize: 10, letterSpacing: 1, marginBottom: 3, display: 'block' } as React.CSSProperties,
  btn: { background: tint(t.blue, 12), color: t.blue, border: `1px solid ${t.blue}`, padding: '6px 14px', borderRadius: 6, cursor: 'pointer', fontFamily: 'inherit', fontSize: 11, fontWeight: 600 } as React.CSSProperties,
...
function tabStyle(sel: boolean): React.CSSProperties {
  return {
    ...
    background: sel ? t.raised : 'transparent', color: sel ? t.bright : t.dim,
    border: `1px solid ${sel ? t.border : 'transparent'}`, borderRadius: 6,
  };
}
...
function QuoteCard({ symbols }: { symbols: string[] }) {
  ...
  if (error) return <div style={{ color: t.red, fontSize: 11 }}>✗ {(error as Error).message}</div>;
  if (!data || Object.keys(data).length === 0) return <div style={S.hint}>No quote data.</div>;

  return (
    <div style={{ marginTop: 8 }}>
      {Object.entries(data).map(([sym, q]: [string, any]) => (
        <div key={sym} style={{ border: `1px solid ${t.border}`, borderRadius: 8, padding: 12, marginBottom: 10, background: t.bg }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
            <span style={{ fontWeight: 800, color: t.bright, fontSize: 14 }}>{sym}</span>
            {q.instrument_token && <span style={pill(t.dim)}>{q.instrument_token}</span>}
          </div>
          <div style={S.grid}>
            <div><span style={S.label}>LTP</span><span style={{ color: t.bright, fontWeight: 700, fontSize: 14 }}>₹{Number(q.last_price ?? 0).toLocaleString('en-IN')}</span></div>
            <div><span style={S.label}>Open</span><span style={{ color: t.bright }}>{Number(q.ohlc?.open ?? 0).toFixed(2)}</span></div>
            <div><span style={S.label}>High</span><span style={{ color: t.bright }}>{Number(q.ohlc?.high ?? 0).toFixed(2)}</span></div>
            <div><span style={S.label}>Low</span><span style={{ color: t.bright }}>{Number(q.ohlc?.low ?? 0).toFixed(2)}</span></div>
            <div><span style={S.label}>Close</span><span style={{ color: t.bright }}>{Number(q.ohlc?.close ?? 0).toFixed(2)}</span></div>
            <div><span style={S.label}>Change</span><span style={{ color: Number(q.net_change ?? 0) >= 0 ? t.green : t.red }}>{Number(q.net_change ?? 0) >= 0 ? '+' : ''}{Number(q.net_change ?? 0).toFixed(2)}%</span></div>
            <div><span style={S.label}>Volume</span><span style={{ color: t.bright }}>{Number(q.volume ?? 0).toLocaleString('en-IN')}</span></div>
            <div><span style={S.label}>OI</span><span style={{ color: t.bright }}>{Number(q.oi ?? 0).toLocaleString('en-IN')}</span></div>
          </div>
          {q.depth && (
            <div style={{ marginTop: 8 }}>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: 4 }}>
                ...
                <tbody>
                  {Array.from({ length: 5 }).map((_, i) => (
                    <tr key={i}>
                      <td style={{ ...S.td, color: t.green }}>{q.depth.buy?.[i]?.price?.toFixed(2) ?? '—'}</td>
                      <td style={{ ...S.td, textAlign: 'right' }}>{q.depth.buy?.[i]?.quantity ?? '—'}</td>
                      <td style={{ ...S.td, color: t.red }}>{q.depth.sell?.[i]?.price?.toFixed(2) ?? '—'}</td>
...
function HistoricalCard() {
  ...
  return (
    <div>
      ...
      {error && <div style={{ color: t.red, fontSize: 11 }}>✗ {(error as Error).message}</div>}
      {data && (
        <div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {data.slice(-200).map((c: any, i: number) => (
                    <tr key={i}>
                      <td style={{ ...S.td, color: t.dim, fontSize: 10 }}>{c[0]}</td>
    ...
    <div>
      <div style={S.card}>
        ...
        {search.data && query.trim().length >= 2 && (
          <div style={{ marginTop: 8, maxHeight: 300, overflow: 'auto', border: `1px solid ${t.border}`, borderRadius: 6 }}>
            {search.data.instruments.map((i) => {
              ...
              return (
                <div key={`${i.exchange}:${i.instrument_token}`} style={{
                  ...
                  borderBottom: `1px solid ${tint(t.border, 50)}`,
                  background: sel ? tint(t.blue, 8) : inWatch ? tint(t.green, 6) : 'transparent',
                }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 2, minWidth: 0 }}>
                    <span style={{ color: t.bright, fontWeight: 700, fontSize: 13 }}>{parseTs(i.tradingsymbol)}</span>
                  ...
                  <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexShrink: 0 }}>
                    ...
                    <span onClick={() => toggleSymbol(sym)} style={{ color: sel ? t.blue : t.dim, fontSize: 18, lineHeight: 1 }}>{sel ? '◉' : '○'}</span>
                  </div>
                </div>
              );
            })}
        ...
        {selected.length > 0 && (
          <div style={{ marginTop: 10, display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            {selected.map((s) => (
              <span key={s} style={{ background: tint(t.blue, 12), color: t.blue, border: `1px solid ${t.blue}`, borderRadius: 999, padding: '2px 8px', fontSize: 10, fontWeight: 600, cursor: 'pointer' }} onClick={() => toggleSymbol(s)}>{s} ✕</span>
            ))}
          </div>
        )}
      </div>
