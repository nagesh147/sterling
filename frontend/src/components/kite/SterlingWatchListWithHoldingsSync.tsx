...
export function SterlingWatchListWithHoldingsSync({
  ...
  const addMissingPositions = useCallback((items: WatchItem[] = positionItems) => {
    ...
    setManualEmpty(false);
    setWatchSnapshot(next);
    // Remount the child watchlist so its internal localStorage-backed hook reads
    // the newly written symbols immediately instead of waiting for a full reload.
  ...
  const refreshAndSyncPositions = useCallback(async () => {
    ...
    try {
      ...
      if (items.length === 0) {
        // Fallback to the backend account-derived sync route. It already reads
        // open positions server-side and marks them with source="position".
  ...
  const showManualEmptyPrompt = watchSnapshot.length === 0 && manualEmpty && !searchActive;
  const hideDefaultEmptyPrompt = watchSnapshot.length === 0 && (!manualEmpty || showManualEmptyPrompt);
