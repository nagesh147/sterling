...
export function NiftyOrbSignalsPanel(){
  ...
  return <div style={{fontSize:11,color:'var(--text-primary)'}}>
    ...
    {!!signals.length&&<div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:10,minWidth:980}}><thead><tr>{['UNDERLYING','STATE','DIR','SPOT','ORB','VWAP','VOL','OPTION','EXPIRY','PREM','SL','TARGET','QTY','STOP RISK','MAX LOSS','DATA'].map(h=><th key={h} style={{textAlign:'left',padding:'7px 6px',borderBottom:'1px solid var(--border)',color:'var(--text-faint)',fontWeight:600,fontSize:9,letterSpacing:'0.06em'}}>{h}</th>)}</tr></thead><tbody>{signals.map(s=><tr key={s.id}>{[
      <b>{s.underlying}</b>,<span style={stateStyle(s.state)}>{s.state}</span>,<span>{s.direction?.toUpperCase()||'—'}</span>,<span>{fmt(s.spot)}</span>,<span>{s.orbHigh==null?'—':`${fmt(s.orbHigh)}/${fmt(s.orbLow)}`}</span>,<span>{fmt(s.vwap)}</span>,<span>{s.volumeRatio==null?'—':`${fmt(s.volumeRatio)}x`}</span>,<span>{s.optionSymbol||'—'}</span>,<span>{s.optionExpiry||'—'}</span>,<span>{fmt(s.optionPremium)}</span>,<span>{fmt(s.stopPremium)}</span>,<span>{fmt(s.targetPremium)}</span>,<span>{s.quantity??'—'}</span>,<span>{s.riskInr==null?'—':`₹${Math.round(s.riskInr).toLocaleString('en-IN')}`}</span>,<span style={{fontWeight:700}}>{s.maxLossInr==null?'—':`₹${Math.round(s.maxLossInr).toLocaleString('en-IN')}`}</span>,<span>{s.dataSource||'—'}</span>
