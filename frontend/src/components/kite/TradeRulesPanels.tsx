 ...
 *
 * They used to be one page with an All / Manual / Automatic filter and a
 * MANUAL / AUTO / MANUAL+AUTO chip on every row. That asked the reader to hold
 * a second axis in their head while scanning a list, and the "All" tab put
 * rules that can never apply to them next to rules that always do.
 *
 * Now the page IS the group. A rule that genuinely governs both kinds of order
 * — the protection mode, the expiry square-off, the time stop — is rendered on
 * both pages, editing the same single value, with a line saying so. That is
 * honest about the backend (both order paths call `protection.arm_position`
 * with the same `cfg.stop_mode`) without making the reader decode a badge.
 */
  ...
  return (
    <PanelCard>
      <PanelHeader
        ...
        description="What happens to a trade you put on by hand from the signal board or the order window. Nothing on this page can block your order — you decide when and what to buy."
        saving={saving}
      />
      ...
      >
        <Field label={FIELDS.protect_manual_orders.label} hint={FIELDS.protect_manual_orders.help}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
            <Switch
              checked={protectOn} label="Protect orders I place by hand"
        ...
        {protectOn ? (
          <ProtectionMode
            value={cfg.stop_mode}
            onChange={(v) => patch({ stop_mode: v }, 'stop_mode', `Protection mode changed to ${v}`)}
            alsoOn="automatic"
          />
        ) : (
          <ConfigNote>
            With protection off, the rest of this page does not apply — your order gets no stop, no
            tick monitor and no expiry square-off, and the board&apos;s SL/TSL columns are a plan you
      ...
      {protectOn && (
        ...
        >
          ...
          />
          <ConfigNote>
            These run over every position the server is tracking, including the ones you placed by
            hand while protection was on. That is what makes a physically-settled stock option safe
            to hold — without the expiry square-off it can go to delivery.
          </ConfigNote>
        </Section>
      )}
...
export function AutomaticRulesPanel() {
  ...
  return (
    <>
      {/* The page's own on/off, so the reader never has to wonder whether any of
          this is live. Everything below is inert until this is armed. */}
      <PanelCard>
        <div style={{
      ...
      <PanelCard>
        <PanelHeader
          ...
          description="What the engine is allowed to open, how big, and what closes it. These apply to whichever engine produced the signal."
          saving={saving}
        />

        <Section
          title="1 · What it may enter"
          description="Filters that can refuse an automatic entry. Blank or zero disables one."
          summary={[cfg.adx_min, cfg.atr_pct_min, cfg.max_spread_pct, cfg.min_oi].filter((v) => v != null).length
            + ((cfg.block_entry_minutes_before_close ?? 0) > 0 ? 1 : 0) + ' active'}
          defaultOpen
        >
        ...
        <Section
          title="6 · Stop trading"
          description="Limits that halt new automatic entries altogether."
