    ...
    <div style={{
      ...
      {mode !== 'minimized' && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '6px 14px', borderBottom: `1px solid ${t.border}`, background: t.headerBg, fontSize: 11, color: t.headDim, flexShrink: 0 }}>
          ...
          {scanning && (
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, color: k.green, fontWeight: 600 }}>
              <span style={{ fontSize: 13, lineHeight: 1 }}>{spinner}</span>
              {data?.scanning_label || 'scanning…'}
            </span>
          )}
