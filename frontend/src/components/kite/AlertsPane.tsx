...
const S: Record<string, React.CSSProperties> = {
  header: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 16px', borderBottom: '1px solid var(--k-surface-hover)' },
  title: { fontSize: 16, color: 'var(--k-text)', fontWeight: 400 },
  actions: { display: 'flex', gap: 12, alignItems: 'center' },
  primaryBtn: { background: 'var(--k-blue-kite)', color: 'var(--k-on-accent)', padding: '8px 16px', borderRadius: 3, border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 500, display: 'flex', alignItems: 'center', gap: 6 },
  searchInputWrapper: { position: 'relative', display: 'flex', alignItems: 'center' },
  searchInput: { border: '1px solid var(--k-border)', borderRadius: 3, padding: '8px 12px 8px 30px', fontSize: 13, color: 'var(--k-text)', outline: 'none', width: 200 },
  searchIcon: { position: 'absolute', left: 10, color: 'var(--k-dim)' },
  ...
  td: { padding: '12px 16px', fontSize: 13, color: 'var(--k-text)', borderBottom: '1px solid var(--k-surface-hover)', verticalAlign: 'top' },
...
function Pill({ type, children }: { type: 'disabled' | 'enabled' | 'simple' | 'ato'; children: React.ReactNode }) {
  const styles = {
    disabled: { color: 'var(--k-red-strong)', background: 'rgba(229, 57, 53, 0.1)' },
    enabled: { color: 'var(--k-green)', background: 'rgba(76, 175, 80, 0.1)' },
    simple: { color: 'var(--k-purple)', background: 'rgba(156, 39, 176, 0.1)' },
    ato: { color: 'var(--k-amber-2)', background: 'rgba(255, 152, 0, 0.1)' },
  };
...
export function AlertsPane() {
  ...
  return (
    <div style={{ background: 'var(--k-bg)', height: '100%', padding: '24px 32px' }}>
      <div style={S.header}>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        ...
        <tbody>
          {(!alerts || alerts.length === 0) && (
            <tr>
              <td colSpan={6} style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--k-dim)', fontSize: 14 }}>
                No alerts found.
              </td>
            </tr>
          )}
          {alerts && alerts.map((a: KiteAlert) => {
            ...
            return (
              <tr key={a.uuid} onClick={() => setEditingAlert(a)} style={{ transition: 'background 0.2s', cursor: 'pointer' }}>
                ...
                <td style={S.td}>
                  <div style={{ color: 'var(--k-text)', fontWeight: 400, marginBottom: 4 }}>{a.name}</div>
                  <div style={{ color: 'var(--k-dim)', fontSize: 11 }}>Last price of {cond}</div>
                </td>
                <td style={S.td}>
                ...
                <td style={{ ...S.td, color: 'var(--k-blue-kite)' }}>
                  {expandedId === a.uuid ? (
