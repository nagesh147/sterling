...
export function SignalImpactCalculator({ data, onBuy, updatedAt }: Props) {
  ...
  return (
    <>
      {/* ── Card 1: Trade Impact Calculator ── */}
      <div style={{ border: `1px solid ${k.border}`, borderRadius: 10, marginBottom: 12, overflow: 'hidden', background: k.bg }}>
        <div style={{ padding: '14px 16px', background: k.surface, borderBottom: `1px solid ${k.border}` }}>
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap', marginTop: 10 }}>
            ...
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 4, border: `1px solid ${k.border}`, borderRadius: 6, padding: '3px 8px', background: k.bg }}>
              <input type="number" value={move} step={5}
                onChange={(e) => setMove(Math.max(1, Number(e.target.value) || stopDist))}
                style={{ width: 48, fontSize: 12, fontWeight: 600, border: 'none', outline: 'none', textAlign: 'right', background: 'transparent', color: k.text }} />
              <span style={{ fontSize: 10, color: k.dim }}>pts</span>
            </div>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, border: `1px solid ${k.border}`, borderRadius: 6, padding: '3px 8px', background: k.bg, marginLeft: 'auto' }}>
              ...
              <input type="number" value={lotsMult} min={1} step={1}
                ...
                style={{ width: 32, fontSize: 12, fontWeight: 600, border: 'none', outline: 'none', textAlign: 'right', background: 'transparent', color: k.text }} />
            </div>
          </div>
        </div>
      ...
      {/* ── Card 2: Premium breakdown ── */}
      {rec && (
        <div style={{ border: `1px solid ${k.border}`, borderRadius: 10, marginBottom: 12, overflow: 'hidden', background: k.bg }}>
          <div style={{ padding: '10px 14px', background: k.surface, borderBottom: `1px solid ${k.border}` }}>
            <span style={{ fontSize: 11, fontWeight: 700, color: k.dim, letterSpacing: 0.4, textTransform: 'uppercase' }}>Premium breakdown (approximate)</span>
            <span style={{ fontSize: 11, color: k.dim }}> — {rec.leg.moneyness} {rec.leg.strike} · ₹{rec.premium.toFixed(2)} total</span>
          </div>
      ...
      {/* ── Card 3: Same move, every strike compared ── */}
      <div style={{ border: `1px solid ${k.border}`, borderRadius: 10, marginBottom: 12, overflow: 'hidden', background: k.bg }}>
        <div style={{ padding: '10px 14px', background: k.surface, borderBottom: `1px solid ${k.border}` }}>
          <span style={{ fontSize: 11, fontWeight: 700, color: k.dim, letterSpacing: 0.4, textTransform: 'uppercase' }}>
      ...
      {/* ── Card 4: Plain-English read ── */}
      {rec && (
        <div style={{ border: `1px solid ${k.border}`, borderRadius: 10, marginBottom: 16, overflow: 'hidden', background: k.surface }}>
          <div style={{ padding: '12px 16px', fontSize: 11.5, color: k.dim, lineHeight: 1.7 }}>
