...
export function PositionsStrip() {
  ...
  return (
    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 6, padding: 14, marginBottom: 14 }}>
      {/* summary row */}
      <div style={{ display: 'flex', gap: 20, marginBottom: open.length > 0 ? 12 : 0, flexWrap: 'wrap' as const }}>
        <div>
          <div style={{ fontSize: 9, color: 'var(--text-faint)', letterSpacing: 1 }}>OPEN</div>
          <div style={{ fontSize: 16, fontWeight: 700, color: open.length > 0 ? 'var(--text-primary)' : 'var(--text-faint)' }}>{open.length}</div>
        </div>
        {open.length > 0 && (
          <div>
            <div style={{ fontSize: 9, color: 'var(--text-faint)', letterSpacing: 1 }}>LIVE P&amp;L</div>
            <div style={{ fontSize: 16, fontWeight: 700, color: totalLivePnl >= 0 ? 'var(--accent)' : 'var(--danger)', fontVariantNumeric: 'tabular-nums' }}>
        ...
        {closed.length > 0 && (
          <div>
            <div style={{ fontSize: 9, color: 'var(--text-faint)', letterSpacing: 1 }}>REALIZED</div>
            <div style={{ fontSize: 16, fontWeight: 700, color: totalRealizedPnl >= 0 ? 'var(--accent)' : 'var(--danger)', fontVariantNumeric: 'tabular-nums' }}>
              {totalRealizedPnl >= 0 ? '+' : ''}${Math.abs(totalRealizedPnl).toFixed(0)}
            </div>
          </div>
        )}
        <div>
          <div style={{ fontSize: 9, color: 'var(--text-faint)', letterSpacing: 1 }}>CLOSED</div>
          <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--text-dim)' }}>{closed.length}</div>
        </div>
      </div>
          ...
          <div key={pos.id} style={{
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', gap: 8, alignItems: 'baseline' }}>
                <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)' }}>{pos.underlying}</span>
                <span style={{ fontSize: 10, color: pos.status === 'partially_closed' ? 'var(--warning)' : 'var(--text-dim)' }}>
            ...
            <div style={{ textAlign: 'right' as const, flexShrink: 0 }}>
              ...
              <button
                ...
                style={{
                  ...
                  color: 'var(--danger)', borderRadius: 3, padding: '2px 8px',
