...
interface OrbConfig {
  enabled: boolean; underlying: string; interval_minutes: number; opening_range_minutes: number;
  entry_start: string; entry_end: string; min_breakout_atr: number; volume_multiplier: number;
  vwap_slope_lookback: number; trend_lookback: number; atr_period: number; stop_buffer_atr: number;
  trail_atr: number; target_r: number; option_moneyness: string; option_steps_itm: number;
  max_risk_inr: number; max_trades_per_day: number; avoid_expiry_day: boolean; expiry_selection: string;
  execution_broker: string; data_source: 'kite' | 'truedata'; max_spread_pct: number;
  min_option_volume: number; min_open_interest: number;
}
...
export function NiftyOrbOptionsSettings() {
  ...
  const { data, isLoading } = useQuery<{ config: OrbConfig }>({ queryKey: ['nifty-orb-options-config'], queryFn: () => api.get('/api/v1/config/nifty-orb-options'), staleTime: 30_000 });
  const [draft, setDraft] = React.useState<Partial<OrbConfig>>({});
  const cfg = { ...(data?.config || {}), ...draft } as OrbConfig;
  const update = useMutation({ mutationFn: (body: Partial<OrbConfig>) => api.put('/api/v1/config/nifty-orb-options', body), onSuccess: result => { qc.setQueryData(['nifty-orb-options-config'], result); setDraft({}); } });
  ...
  return <>
    <Section title="NIFTY ORB + VWAP OPTIONS">
      <div style={{ padding: '10px 12px', background: 'var(--t-bg2)', border: '1px solid var(--t-border)', borderRadius: 5, marginBottom: 14, fontSize: 10, color: 'var(--t-dim)', lineHeight: 1.6 }}>
        NIFTY 50 generates direction. CE/PE is only the execution vehicle. Strategy ON/OFF controls signal generation; Paper/Live and Manual/Auto remain universal Trading Mode controls.
      </div>
      ...
      <Field label="DATA SOURCE" hint="Market data only; execution remains Kite"><select value={cfg.data_source} onChange={e => set('data_source', e.target.value)} style={inputStyle}><option value="kite">Zerodha Kite</option><option value="truedata">TrueData</option></select></Field>
    ...
    <Section title="OPTION LIQUIDITY">
      <Field label="MAX BID/ASK SPREAD %" hint="Entries are rejected when the quoted spread exceeds this percentage of mid."><input type="number" step="0.1" min="0.1" max="10" value={cfg.max_spread_pct} onChange={e => set('max_spread_pct', Number(e.target.value))} style={inputStyle}/></Field>
    ...
    <Section title="EXECUTION">
      <Field label="EXECUTION BROKER"><input value="ZERODHA KITE" readOnly style={{ ...inputStyle, color: 'var(--t-blue)' }}/></Field>
      <div style={{ fontSize: 9, color: 'var(--t-dim)', lineHeight: 1.6 }}>ORB does not own Paper/Live or Manual/Auto. Those controls are inherited from the universal Trading Mode and the active Kite account. Automatic ORB entries use the same idempotency, live-safety and position-protection path as SuperTrend.</div>
    </Section>
