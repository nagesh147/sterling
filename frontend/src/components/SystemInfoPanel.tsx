...
const S: Record<string, React.CSSProperties> = {
  ...
  val: { fontSize: 13, color: t.text, fontWeight: 600 },
  stack: {
    background: t.bg, border: `1px solid ${t.border}`, borderRadius: 4,
    padding: '8px 12px', fontSize: 11, color: t.dim, fontFamily: 'Courier New, monospace',
    marginBottom: 12,
  },
  row: { display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 8 },
  chip: {
    padding: '2px 8px', borderRadius: 3, fontSize: 11,
    background: t.raised, border: `1px solid ${t.border}`, color: t.text,
  },
  chipGreen: {
    padding: '2px 8px', borderRadius: 3, fontSize: 11,
    background: '#44cc8818', border: '1px solid #44cc8844', color: t.green,
  },
  dsRow: {
  ...
  btnOrange: {
    background: '#1a1500', color: t.amber, border: `1px solid ${t.amber}`,
    padding: '5px 10px', borderRadius: 3, cursor: 'pointer', fontFamily: 'inherit', fontSize: 11,
  },
  status: { fontSize: 11, color: t.dim },
  success: { fontSize: 11, color: t.green },
  error: { fontSize: 11, color: t.red },
};
...
export function SystemInfoPanel() {
  ...
  if (isLoading) return (
    <div style={S.card}><div style={{ color: t.dim, fontSize: 12 }}>Loading system info…</div></div>
  );
  if (!data) return null;
  ...
  return (
    <div style={S.card}>
      ...
      <div style={S.dsRow}>
        ...
        {dsData && (
          <span style={{ color: dsData.reachable ? t.green : t.red, fontSize: 11 }}>
      ...
      <div style={S.grid}>
        ...
        <div style={S.cell}>
          ...
          <span style={{ ...S.val, color: data.environment === 'production' ? t.amber : t.green }}>
        ...
        <div style={S.cell}>
          ...
          <span style={{ ...S.val, color: data.paper_trading ? t.green : t.red }}>
        ...
        <div style={S.cell}>
          ...
          <span style={{ ...S.val, fontSize: 11, color: t.dim, fontFamily: 'monospace' }}>
