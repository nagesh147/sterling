...
export function MiniGridPane({ paneIndex, baseCandles, activeIndicators, params, tv, isLogScale, chartsRef, syncGuardRef }: MiniGridPaneProps) {
  ...
  useEffect(() => {
    ...
    const addST = (period: number, mult: number) => {
      ...
      // whitespace, so those drew two crossing lines per indicator. See supertrendRuns.
      supertrendRuns(stData, times).forEach((run) => {
        chart.addSeries(LineSeries, { color: (run.up ? tv.green : tv.red) + '66', lineWidth: 1, priceLineVisible: false, lastValueVisible: false }).setData(run.points as any);
      });
    };
